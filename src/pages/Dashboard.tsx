 ...
 * Keep these out of the critical render path.
 */
...
export default function Dashboard() {
  ...
  useEffect(() => {
    // Defer demo seeding so first paint happens sooner (avoid blocking).
    const cancel = defer(() => {
  ...
  useEffect(() => {
    // Mount heavy panels after initial paint (dashboard best practice).
    // Order matters: realtime first, wallet/leaderboard next, charts last.
    const cancels: Array<() => void> = [];

    cancels.push(defer(() => setShowRealtime(true), 800));
    cancels.push(defer(() => setShowWallet(true), 1200));
    cancels.push(defer(() => setShowLeaderboard(true), 1400));
    cancels.push(defer(() => setShowCharts(true), 1800));
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Advanced Charts (deferred + lazy; typically largest deps) */}
        <div className="mb-8">
          {showCharts ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {/* Right Column - Wallet, Token Balance, Leaderboard */}
